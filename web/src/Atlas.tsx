...
import { aggregateLendingUsdBn, LendingHeatGlobe } from "./LendingHeatGlobe";
...
import {
  ...
  WB_INCOME_OPTIONS,
  formatUsdBnTotal,
  summarizeMarketLending,
} from "./data/countryClassifications";
import {
  formatUsdCompact,
  summarizeInvestedPortfolio,
...
/** 大屏地图区域缩放（常用按钮上方） */
type BigScreenMapRegion = "se-asia" | "latam" | "central-asia" | "africa" | "south-asia";
const BIG_SCREEN_MAP_REGIONS: { id: BigScreenMapRegion; label: string }[] = [
...
function BigScreenOverlay({
  ...
  /** 大屏区域缩放 id；空字符串=全球 */
  mapRegion?: BigScreenMapRegion | "";
}) {
  const theme = useHostTheme();
  const [imfDev, setImfDev] = useCanvasState<string>("screenImfDev1", "");
  const [wbIncome, setWbIncome] = useCanvasState<string>("screenWbInc1", "");
  ...
  const marketSummary = useMemo(
    () =>
      summarizeMarketLending({
        imfDev,
        wbIncome,
        restrictCodes: regionZoomCodes,
        restrictLabel: regionLabel || undefined,
      }),
    [imfDev, wbIncome, regionZoomCodes, regionLabel],
  );
  const investedSummary = useMemo(
    () => summarizeInvestedPortfolio(aggregateLendingUsdBn(), regionZoomCodes),
    [regionZoomCodes],
  );
  const both = showMarket && showInvested && !ecoOn;
  const marketEco = showMarket && ecoOn;
  const investedOn = showInvested && !ecoOn;
  ...
  const subtitle = marketEco
    ? `市场在贷 × ${INSTITUTION_TYPE_LABEL[ecoType as InstitutionType]}样本数`
    : ecoOn
      ? `${INSTITUTION_TYPE_LABEL[ecoType as InstitutionType]}分布`
      : both
        ? "市场在贷 × 已投平台"
  ...
  const showMarketBar = showMarket || marketEco;
  const showInvestedBar = investedOn;
  const kpiNum = bare ? 20 : 24;

  const filterBar =
    showMarketBar || showInvestedBar ? (
      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
          alignItems: "center",
          gap: 12,
          padding: bare ? "8px 12px" : "10px 14px",
          borderRadius: bare ? 0 : 10,
          border: bare ? "none" : `1px solid ${theme.stroke.tertiary}`,
          background: bare ? "rgba(255,255,255,0.92)" : theme.bg.elevated,
        }}
      >
        {showMarketBar ? (
          <div style={{ minWidth: 180, flex: "1 1 180px" }}>
            <div style={{ fontSize: 11, color: theme.text.tertiary, marginBottom: 2 }}>
              {marketSummary.label} · 在贷余额合计
            </div>
            <div
              style={{
                fontSize: kpiNum,
            ...
            >
              {formatUsdBnTotal(marketSummary.totalUsdBn)}
            </div>
            <div style={{ fontSize: 11, color: theme.text.secondary, marginTop: 2 }}>
              覆盖 {marketSummary.countryCount} 国 · 有数据 {marketSummary.withDataCount} 国
            </div>
          </div>
        ) : null}
        ...
        {showInvestedBar ? (
          <>
            <div style={{ minWidth: 180, flex: "1 1 180px" }}>
              ...
              >
                {formatUsdBnTotal(investedSummary.marketUsdBn)}
              </div>
              <div style={{ fontSize: 11, color: theme.text.secondary, marginTop: 2 }}>
                {investedSummary.countryCount} 国
              </div>
            </div>
            <div style={{ minWidth: 180, flex: "1 1 180px" }}>
              <div style={{ fontSize: 11, color: theme.text.tertiary, marginBottom: 2 }}>
              ...
              >
                {formatUsdCompact(investedSummary.platformOutstandingUsd)}
              </div>
              <div style={{ fontSize: 11, color: theme.text.secondary, marginTop: 2 }}>
                {investedSummary.platformCount} 个平台 · 基金投资{" "}
                {formatUsdCompact(investedSummary.investmentUsd)}
              </div>
            </div>
          </>
        ) : null}
        ...
        {showMarketBar ? (
          <Row gap={10} wrap align="center" style={{ flex: "2 1 320px" }}>
            <Stack gap={4} style={{ minWidth: 150, flex: "1 1 150px" }}>
              <Text size="small" tone="tertiary">
                IMF 发展标签
              </Text>
              <Select value={imfDev} onChange={setImfDev} options={[...IMF_DEV_OPTIONS]} />
            </Stack>
            <Stack gap={4} style={{ minWidth: 160, flex: "1 1 160px" }}>
              <Text size="small" tone="tertiary">
                世行收入分组
              </Text>
              <Select value={wbIncome} onChange={setWbIncome} options={[...WB_INCOME_OPTIONS]} />
            </Stack>
            {imfDev || wbIncome ? (
              <Button
                variant="secondary"
  ...
  const globe = (
    <CombinedHeatGlobe
      ...
      activeCountryCodes={showMarket || marketEco ? marketSummary.codes : regionZoomCodes}
  ...
  const frame = bare ? (
    ...
    >
      {filterBar ? (
        <div style={{ flex: "0 0 auto", zIndex: 6, position: "relative" }}>{filterBar}</div>
      ) : null}
      <div style={{ flex: "1 1 0", minHeight: 0, position: "relative" }}>{globe}</div>
    </div>
  ) : (
...
function BigScreen() {
  ...
  const [mapRegion, setMapRegion] = useCanvasState<BigScreenMapRegion | "">("screenMapReg1", "");
      ...
      <Row gap={6} wrap align="center">
        ...
        {BIG_SCREEN_MAP_REGIONS.map((r) => (
          <FilterChip
            key={r.id}
            label={r.label}
            active={mapRegion === r.id && onMap}
            clearable
            onClick={() => {
              if (showRoster || showCompare) leaveSpecial();
              setMapRegion(mapRegion === r.id ? "" : r.id);
            }}
          />
        ))}
        {mapRegion ? (
